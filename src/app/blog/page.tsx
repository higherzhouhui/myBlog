...
export default function Blog() {
  ...
  const handleToTeam = (item: BlogListInterface) => {
    router.push(`/blogDetail/${item.id}/${item.title}`);
  };
  ...
  return (
    <Box sx={{ padding: Sm ? "0 12px" : "12px 0" }}>
      ...
      <Typography
        component="div"
        sx={{ fontSize: 14, mt: 2, fontWeight: "bold" }}
        color="primary"
      >
        博主力推
      </Typography>
              ...
              <MyCard>
                ...
                >
                  <Typography
                    component="div"
                    security="h2"
                    sx={{ color: "secondary.dark", fontSize: 14, pr: 1 }}
                  >
                    作者：{item.creator}
                ...
                <Typography color={"GrayText"} security="desc">
                  {item.lookNum} 次浏览
                </Typography>
              </MyCard>
            </SwiperSlide>
      ...
      {loading ? (
        <Grid container spacing={2}>
          {[...Array(Sm ? 1 : 4)].map((item, index: number) => {
            return (
              <Grid item xs={Sm ? 12 : 3} key={index}>
                <Skeleton
